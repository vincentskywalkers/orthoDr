...
orthoDr_surv <- function(x, y, censor, method = "dm", ndr = ifelse(method == "forward", 1, 2),
                         ...
                         keep.data = FALSE,
                         control = list(),
                         # Diag_Entry = NULL, #diagnal entries for getting upper block diagnal matrix
                         maxitr = 500,
                         verbose = FALSE,
                         ncore = 0)
{
  ...
  if (method == "dn")
  {
    ...
    # Diag_Entry = c(1, 3)
    # transform_mat = kronecker(t(solve(fit$B[Diag_Entry, ])), diag(P))
    # pick_loc = rep(Diag_Entry, each = ndr) + P*c(0:(ndr-1))
    # pick_mat = transform_mat[-pick_loc, ]
    # B_Var = surv_dn_var(fit$B, X, Phit, Fail.Ind, bw, control$epsilon, pick_mat, ncore)
    # print(sqrt(diag(B_Var)))
  }
  ...
  if (method == "dm")
  {
    ...
    # B_Var = surv_dm_var(fit$B, X, Phit, Fail.Ind, bw, control$epsilon, pick_mat, ncore)
    # print(sqrt(diag(B_Var)))
  }
