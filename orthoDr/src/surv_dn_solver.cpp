...
/*
arma::vec surv_dn_ee_vec(const arma::mat& B,
                         const arma::mat& X,
                         const arma::mat& Phit,
                         const arma::vec& Fail_Ind,
                         double bw,
                         int ncore)
{
  ...
  return vectorise(EE/N);
}



arma::mat surv_dn_A_Cov(const arma::mat& B,
                        const arma::mat& X,
                        const arma::mat& Phit,
                        const arma::vec& Fail_Ind,
                        double bw,
                        int ncore)
{
  ...
  arma::mat A_Cov(P*P, P*P, arma::fill::zeros);
  arma::vec A(P*P);
  ...
  for(int j=0; j < nFail; j++)
  {
    ...
    if (weights > 0)
    {
      A = vectorise(Phit.col(j)*(X.row(fail_j_ind) - TheCond/weights));
      A_Cov += A * A.t();
    }
  }
...
arma::mat surv_dn_var(arma::mat B,
                      const arma::mat& X,
                      const arma::mat& Phit,
                      const arma::vec& Fail_Ind,
                      double bw,
                      double epsilon,
                      const arma::mat& Pick_Mat,
                      int ncore)
{
  // varaicne formular
  ...
  arma::mat Cov_A = surv_dn_A_Cov(B, X, Phit, Fail_Ind, bw, ncore);

  arma::mat Gn(P*P, P*ndr);
  arma::vec EE_vec0 = surv_dn_ee_vec(B, X, Phit, Fail_Ind, bw, ncore);
  ...
  for (int j = 0; j < ndr; j++)
    for (int i = 0; i < P; i++)
    {
      ...
      Gn.col(j*P + i) = (surv_dn_ee_vec(NewB, X, Phit, Fail_Ind, bw, ncore) - EE_vec0)/epsilon;

      NewB(i, j) = temp;
    }

  //Rcout << "A: " << std::setprecision(5) << Cov_A << std::endl;
  //Rcout << "GG: " << std::setprecision(5) << (Gn.t()*Gn) << std::endl;
  //Rcout << "GG inv: " << std::setprecision(5) << (Gn.t()*Gn).i() << std::endl;
  //Rcout << "GG eigen: " << std::setprecision(5) << eig_sym(Gn.t()*Gn) <<std::endl;

  arma::mat GP = Gn * Pick_Mat.t();

  // Rcout << "PGGP eigen: " << std::setprecision(5) << eig_sym(GP.t()*GP) <<std::endl;

  return ( (GP.t()*GP).i() * (GP.t() * Cov_A * GP) * (GP.t()*GP).i() );
}
