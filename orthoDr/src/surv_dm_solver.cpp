...
/*
arma::vec surv_dm_ee_vec(const arma::mat& B,
                         const arma::mat& X,
                         const arma::mat& Phit,
                         const arma::vec& Fail_Ind,
                         double bw,
                         int ncore)
{
  ...
  return vectorise(TheIntegration/N);
}



arma::mat surv_dm_A_Cov(const arma::mat& B,
                        const arma::mat& X,
                        const arma::mat& Phit,
                        const arma::vec& Fail_Ind,
                        double bw,
...
arma::mat surv_dm_var(arma::mat B,
                      const arma::mat& X,
                      const arma::mat& Phit,
                      const arma::vec& Fail_Ind,
                      double bw,
                      ...
                      int ncore)
{
  ...
  arma::mat Cov_A = surv_dm_A_Cov(B, X, Phit, Fail_Ind, bw, ncore);

  arma::mat Gn(P*P, P*ndr);
  arma::vec EE_vec0 = surv_dm_ee_vec(B, X, Phit, Fail_Ind, bw, ncore);
  ...
  for (int j = 0; j < ndr; j++)
    for (int i = 0; i < P; i++)
    {
      ...
      Gn.col(j*P + i) = (surv_dm_ee_vec(NewB, X, Phit, Fail_Ind, bw, ncore) - EE_vec0)/epsilon;
